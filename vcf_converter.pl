...
if ($format eq 'bayescan') {
    ...
    $pid = fork();
    if ($pid == 0) {
        open STDOUT, ">$name.arp0123";
        vcf_2_arlequin_0123($vcf, $popmap, $order);
        exit 0;
    }
    until(waitpid(-1,WNOHANG) == -1){sleep 1;}
} else {
...
sub vcf_2_structure {
    ...
    foreach my $pop (@$order){
        ...
        foreach my $ind (@{$pops->{$pop}}) {
            ...
            my @alt_gts = ();
            map {
                my $geno = $_->{'ind'}->{$ind}->{'geno'};
                my $ref  = $_->{'refb'};
                my $alt  = $_->{'altb'};
                my $gt   = gencode_from_gt($ref,$alt,$geno);
         ...
         foreach my $ind (@{$pops->{$pop}}) {
            ...
            map {
                ...
                if ($g[0] eq '.') {
                    ...
                    $g1 .= ' ' . $sfs_num{$_->{$ref_alt{$g[0]}}};
                    $g2 .= ' ' . $sfs_num{$_->{$ref_alt{$g[1]}}};
                }
